...
const SectionCard = ({ title, icon: Icon, children, action }) => (
    <section className="rounded-2xl border border-[var(--border2)] bg-[var(--surface)] shadow-[var(--shadow)] backdrop-blur-md transition-all hover:shadow-[0_8px_40px_rgba(var(--accent-rgb),0.08)] overflow-hidden">
        <div className="flex items-center justify-between gap-3 border-b border-[var(--border)] bg-[linear-gradient(90deg,rgba(var(--accent-rgb),0.04),transparent)] px-6 py-4">
            <div className="flex items-center gap-2">
                <div className="inline-flex h-10 w-10 items-center justify-center rounded-xl border border-[rgba(var(--accent-rgb),0.2)] bg-[rgba(var(--accent-rgb),0.08)] shadow-inner">
                    <Icon className="h-5 w-5 text-[var(--accent)]" />
                </div>
                <h2 className="font-syne text-[1.1rem] font-bold tracking-tight text-[var(--text)]">{title}</h2>
            </div>
            {action}
        </div>
    ...
    return (
        <div className="relative z-10 min-h-[calc(100vh-4.25rem)] bg-transparent py-10">
            <div className="nova-main space-y-7">
                <div className="relative overflow-hidden rounded-3xl border border-[var(--border2)] bg-[var(--surface)] p-8 shadow-[var(--shadow)]">
                    <div className="absolute inset-0 bg-[linear-gradient(120deg,rgba(var(--accent-rgb),0.12),transparent_40%)] pointer-events-none"></div>
                    <div className="relative z-10 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                        <div>
                            <p className="font-mono text-xs uppercase tracking-[0.2em] text-[var(--accent)] font-semibold">TenaMed Command Center</p>
                            <h1 className="mt-2 font-syne text-3xl font-bold tracking-tight text-[var(--text)] md:text-4xl">Admin Dashboard</h1>
                            <p className="mt-3 max-w-2xl text-sm text-[var(--text2)] leading-relaxed">
                                Centralized control for platform compliance, OCR monitoring, verification workflows, and operational intelligence.
                            </p>
                        </div>
                        <div className="flex flex-wrap items-center gap-3">
                            <Link to="/admin/medical-verification" className="btn-secondary rounded-xl px-5 py-2.5 text-sm font-semibold border border-[var(--border2)] bg-[var(--surface2)] text-[var(--text)] hover:bg-[var(--surface3)] transition-colors shadow-sm">
                                Open Verification Queue
                            </Link>
                            <button type="button" onClick={loadDashboard} className="btn-primary rounded-xl px-5 py-2.5 text-sm font-semibold shadow-md">
                                Refresh Dashboard
                            </button>
                        </div>
                    </div>
                ...
                {errorMsg ? <div className="rounded-xl border border-rose-500/40 bg-rose-500/10 px-4 py-3 text-sm font-medium text-[var(--danger)] shadow-sm">{errorMsg}</div> : null}
                {successMsg ? <div className="rounded-xl border border-emerald-500/40 bg-emerald-500/10 px-4 py-3 text-sm font-medium text-[var(--success)] shadow-sm">{successMsg}</div> : null}
                ...
                <section className="grid grid-cols-1 gap-5 sm:grid-cols-2 xl:grid-cols-3">
                    {kpiCards.map((card) => {
                        ...
                        return (
                            <article key={card.label} className="group rounded-2xl border border-[var(--border2)] bg-[var(--surface)] p-6 shadow-[var(--shadow)] transition-all hover:-translate-y-1 hover:shadow-[0_12px_30px_rgba(var(--accent-rgb),0.12)] hover:border-[var(--border3)]">
                                <div className="mb-4 flex items-center justify-between">
                                    <p className="text-sm font-medium text-[var(--text2)]">{card.label}</p>
                                    <div className="inline-flex h-10 w-10 items-center justify-center rounded-xl border border-[rgba(var(--accent-rgb),0.15)] bg-[rgba(var(--accent-rgb),0.06)] transition-colors group-hover:bg-[rgba(var(--accent-rgb),0.1)]">
                                        <Icon className="h-5 w-5 text-[var(--accent)]" />
                                    </div>
                                </div>
                                <div className="text-3xl font-bold tracking-tight text-[var(--text)]">{isLoading ? '...' : card.value}</div>
                            </article>
                        );
                    })}
                </section>
                                ...
                                {hospitalSearchResults.length ? (
                                    <ul className="mt-2 max-h-40 space-y-2 overflow-y-auto text-sm">
                                        {hospitalSearchResults.slice(0, 12).map((h) => (
                                            <li key={h.id} className="flex flex-wrap items-center justify-between gap-2 rounded-lg border border-[var(--border2)] bg-[var(--bg2)] px-3 py-2 transition-colors hover:border-[var(--border3)]">
                                                <span className="font-medium text-[var(--text2)]">{h.name || h.id}</span>
                                                <button
                                                    type="button"
                                                    onClick={() => {
                                                    ...
                                                    className="shrink-0 rounded-md border border-[rgba(var(--accent-rgb),0.35)] px-3 py-1.5 text-xs font-semibold text-[var(--accent)] transition-colors hover:bg-[rgba(var(--accent-rgb),0.12)]"
                                                >
                                                    Use ID
                                                </button>
                                            </li>
                                ...
                                {pharmacySearchResults.length ? (
                                    <ul className="mt-2 max-h-40 space-y-2 overflow-y-auto text-sm">
                                        {pharmacySearchResults.slice(0, 12).map((p) => (
                                            <li key={p.id} className="flex flex-wrap items-center justify-between gap-2 rounded-lg border border-[var(--border2)] bg-[var(--bg2)] px-3 py-2 transition-colors hover:border-[var(--border3)]">
                                                <span className="font-medium text-[var(--text2)]">{p.name || p.legalName || p.id}</span>
                                                <button
                                                    type="button"
                                                    onClick={() => {
                ...
                <div className="grid grid-cols-1 gap-6 xl:grid-cols-2">
                    <SectionCard title="Hospital Management" icon={Building2}>
                        <div className="space-y-3">
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                <button type="button" disabled={isActionLoading} onClick={() => executeAdminAction({ entityType: 'hospital', entityId: hospitalId, actionType: 'approve' })} className="rounded-lg border border-[rgba(var(--success-rgb),0.4)] px-3 py-2 text-sm font-medium text-[var(--success)] hover:bg-[rgba(var(--success-rgb),0.1)] disabled:opacity-50 transition-colors shadow-sm">Approve</button>
                                <button type="button" disabled={isActionLoading} onClick={() => executeAdminAction({ entityType: 'hospital', entityId: hospitalId, actionType: 'reject' })} className="rounded-lg border border-[rgba(var(--danger-rgb),0.4)] px-3 py-2 text-sm font-medium text-[var(--danger)] hover:bg-[rgba(var(--danger-rgb),0.1)] disabled:opacity-50 transition-colors shadow-sm">Reject</button>
                                <button type="button" disabled={isActionLoading} onClick={() => executeAdminAction({ entityType: 'hospital', entityId: hospitalId, actionType: 'suspend' })} className="rounded-lg border border-[rgba(var(--warn-rgb),0.4)] px-3 py-2 text-sm font-medium text-[var(--warn)] hover:bg-[rgba(var(--warn-rgb),0.1)] disabled:opacity-50 transition-colors shadow-sm">Suspend</button>
                                <button type="button" disabled={isActionLoading} onClick={() => executeAdminAction({ entityType: 'hospital', entityId: hospitalId, actionType: 'unsuspend' })} className="rounded-lg border border-[rgba(var(--accent-rgb),0.4)] px-3 py-2 text-sm font-medium text-[var(--accent)] hover:bg-[rgba(var(--accent-rgb),0.1)] disabled:opacity-50 transition-colors shadow-sm">Unsuspend</button>
                            </div>
                        </div>
                    </SectionCard>
                    ...
                    <SectionCard title="Pharmacy Management" icon={Store}>
                        <div className="space-y-3">
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                <button type="button" disabled={isActionLoading} onClick={() => executeAdminAction({ entityType: 'pharmacy', entityId: pharmacyId, actionType: 'approve' })} className="rounded-lg border border-[rgba(var(--success-rgb),0.4)] px-3 py-2 text-sm font-medium text-[var(--success)] hover:bg-[rgba(var(--success-rgb),0.1)] disabled:opacity-50 transition-colors shadow-sm">Approve</button>
                                <button type="button" disabled={isActionLoading} onClick={() => executeAdminAction({ entityType: 'pharmacy', entityId: pharmacyId, actionType: 'reject' })} className="rounded-lg border border-[rgba(var(--danger-rgb),0.4)] px-3 py-2 text-sm font-medium text-[var(--danger)] hover:bg-[rgba(var(--danger-rgb),0.1)] disabled:opacity-50 transition-colors shadow-sm">Reject</button>
                                <button type="button" disabled={isActionLoading} onClick={() => executeAdminAction({ entityType: 'pharmacy', entityId: pharmacyId, actionType: 'suspend' })} className="rounded-lg border border-[rgba(var(--warn-rgb),0.4)] px-3 py-2 text-sm font-medium text-[var(--warn)] hover:bg-[rgba(var(--warn-rgb),0.1)] disabled:opacity-50 transition-colors shadow-sm">Suspend</button>
                                <button type="button" disabled={isActionLoading} onClick={() => executeAdminAction({ entityType: 'pharmacy', entityId: pharmacyId, actionType: 'unsuspend' })} className="rounded-lg border border-[rgba(var(--accent-rgb),0.4)] px-3 py-2 text-sm font-medium text-[var(--accent)] hover:bg-[rgba(var(--accent-rgb),0.1)] disabled:opacity-50 transition-colors shadow-sm">Unsuspend</button>
                            </div>
                        </div>
                    </SectionCard>
                </div>
                ...
                >
                    <div className="mb-4 rounded-xl border border-[var(--border2)] bg-[rgba(var(--accent-rgb),0.06)] p-4 text-sm text-[var(--text2)] shadow-sm">
                        <div className="font-semibold text-[var(--accent)]">Admin write API</div>
                        <div className="mt-1.5 opacity-90">
                            POST <span className="font-mono text-[11px] font-semibold text-[var(--text)] bg-[var(--surface)] px-1.5 py-0.5 rounded border border-[var(--border2)]">/api/admin/medicines</span> • PUT <span className="font-mono text-[11px] font-semibold text-[var(--text)] bg-[var(--surface)] px-1.5 py-0.5 rounded border border-[var(--border2)]">/api/admin/medicines/{'{id}'}</span> • DELETE <span className="font-mono text-[11px] font-semibold text-[var(--text)] bg-[var(--surface)] px-1.5 py-0.5 rounded border border-[var(--border2)]">/api/admin/medicines/{'{id}'}</span>
                        </div>
                    </div>
                            ...
                            <div className="space-y-3">
                                ...
                                <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
                                    ...
                                    <button
                                        ...
                                        className="rounded-xl border border-[var(--border2)] bg-[var(--bg2)] px-4 py-2 text-sm font-semibold text-[var(--text)] hover:bg-[var(--bg3)] transition-colors"
                                    >
                                        Clear form
                                    </button>
                                </div>
                                ...
                                <div className="grid grid-cols-1 gap-3 md:grid-cols-3 mt-2">
                                    <button type="button" disabled={isMedicineLoading} onClick={createMedicine} className="rounded-lg border border-[rgba(var(--success-rgb),0.4)] px-3 py-2.5 text-sm font-semibold text-[var(--success)] hover:bg-[rgba(var(--success-rgb),0.1)] disabled:opacity-50 transition-colors shadow-sm">Create Medicine</button>
                                    <button type="button" disabled={isMedicineLoading} onClick={updateMedicine} className="rounded-lg border border-[rgba(var(--accent-rgb),0.4)] px-3 py-2.5 text-sm font-semibold text-[var(--accent)] hover:bg-[rgba(var(--accent-rgb),0.1)] disabled:opacity-50 transition-colors shadow-sm">Update Medicine</button>
                                    <button type="button" disabled={isMedicineLoading} onClick={deleteMedicine} className="rounded-lg border border-[rgba(var(--danger-rgb),0.4)] px-3 py-2.5 text-sm font-semibold text-[var(--danger)] hover:bg-[rgba(var(--danger-rgb),0.1)] disabled:opacity-50 transition-colors shadow-sm">Delete Medicine</button>
                                </div>
                            </div>
                        </div>
                    </div>
                                ...
                                <article key={entry.id} className="rounded-xl border border-[var(--border2)] bg-[var(--bg)] p-3">
                                    <div className="flex flex-wrap items-center justify-between gap-3">
                                        ...
                                        <div className="flex items-center gap-2">
                                            ...
                                            {entry.highRisk ? (
                                                <span className="inline-flex items-center gap-1 rounded-full border border-rose-500/30 bg-rose-500/10 px-2 py-1 text-xs font-medium text-[var(--danger)]">
                                                    <ShieldAlert className="h-3.5 w-3.5" />
                                                    High risk
                                                </span>
                                            ) : (
                                                <span className="inline-flex items-center gap-1 rounded-full border border-emerald-500/30 bg-emerald-500/10 px-2 py-1 text-xs font-medium text-[var(--success)]">
                ...
                <section className="grid grid-cols-1 gap-4 rounded-2xl border border-[var(--border2)] bg-[var(--surface)] p-6 md:grid-cols-3 shadow-[var(--shadow)]">
                    <div className="flex items-start gap-3">
                        <UserCheck className="mt-0.5 h-5 w-5 text-[var(--success)]" />
                        <div>
                            <h3 className="text-sm font-semibold text-[var(--text)]">Verification throughput</h3>
                            <p className="mt-0.5 text-xs text-[var(--text2)]">Orders tracked by backend: <span className="font-semibold text-[var(--success)]">{isLoading ? '...' : formatNumber(dashboard?.totalOrders)}</span></p>
                        </div>
                    </div>
                    <div className="flex items-start gap-3">
                        <XCircle className="mt-0.5 h-5 w-5 text-[var(--danger)]" />
                        <div>
                            <h3 className="text-sm font-semibold text-[var(--text)]">Rejection volume</h3>
                            <p className="mt-0.5 text-xs text-[var(--text2)]">OCR failed: <span className="font-semibold text-[var(--danger)]">{isLoading ? '...' : formatNumber(ocrStats?.failedCount)}</span></p>
                        </div>
                    </div>
                    <div className="flex items-start gap-3">
                        <Activity className="mt-0.5 h-5 w-5 text-[var(--accent)]" />
                        <div>
                            <h3 className="text-sm font-semibold text-[var(--text)]">Compliance status</h3>
                            <p className="mt-0.5 text-xs text-[var(--text2)]">Current OCR confidence: <span className="font-semibold text-[var(--accent)]">{isLoading ? '...' : `${Number(ocrStats?.avgConfidence || 0).toFixed(2)}%`}</span></p>
                        </div>
                    </div>
                </section>
            </div>
